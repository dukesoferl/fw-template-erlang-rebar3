AC_DEFUN([FW_TEMPLATE_ERLANG_REBAR3],
[
  FW_ERL_APP_NAME=${FW_ERL_APP_NAME-${FW_PACKAGE_NAME}}
  FW_SUBST_PROTECT(FW_ERL_APP_NAME)

  AC_REQUIRE([FW_TEMPLATE_ERLANG_REBAR3_ENABLE_ERLRC])
  ...
  AC_MSG_CHECKING([looking for rebar3])

  AC_PATH_PROG([REBAR3], [rebar3], ,[.:$PATH])
  if test "x$REBAR3" = x
  then
    AC_MSG_ERROR([cant find rebar3])
    exit 1
    fi
    AC_ARG_VAR(REBAR3, [rebar3 build tool])
  AC_SUBST([REBAR3])
